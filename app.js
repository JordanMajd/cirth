...
// character indices is svg number - 1
  ...
  title.innerHTML = transliterate(title.innerText).runes;
  ...
  runeInput.onkeyup = function(event){
    ...
    var result = transliterate(textValue);
    ...
    runeContainer.innerHTML = result.runes;
  ...
  function transliterate(textValue) {
    ...
    // replace w/ svg
    runeMap.forEach(function(img_index, key) {
      textValue = textValue.replaceAll(key, img_index + ",");
    });
    ...
    if(invalidChars.length > 0) {
      invalidTag = `<span class="invalid">Invalid characters removed (${invalidChars.join(",")}).<span/>`;
    }
     
    return { runes: runeTags, errors: invalidTag };
  }
}());